...
async def run_inference(prompt: str, file: UploadFile = File(...)):
    try:
...
@app.post("/general")
async def run_smol_general(obj: str, file: UploadFile = File(...)):
    try:
        try:
            response = await run_inference(prompt=get_pretext(obj=obj, general=True) + GENERAL_CONTEXT_PROMPT, file=file)
            if response is None:
...
@app.post("/specific")
async def run_smol_specific(obj: str, file: UploadFile = File(...)):
    try:
        try:
            response = await run_inference(prompt=get_pretext(obj=obj, general=False) + CONTEXT_SPECIFIC_PROMPT, file=file)
            if response is None:
...
async def run_smol_retrieval(
    ...
    try:
        try:
            cropped_response = await run_inference(prompt=get_pretext(obj=obj, general=False) + GENERAL_CONTEXT_PROMPT, file=cropped)
            try:
                specific_generated_json = json.loads(cropped_response)
            except:
                specific_generated_json = {"raw_output": cropped_response}
            
            full_response = await run_inference(prompt=get_pretext(obj=obj, general=True) + CONTEXT_SPECIFIC_PROMPT, file=full)
            try:
                context_generated_json = json.loads(full_response)
            except:
                context_generated_json = {"raw_output": full_response}
            
            parsed = {"general" : context_generated_json, "specific" : specific_generated_json}
            return JSONResponse(content=parsed)
        except json.JSONDecodeError:
            return JSONResponse(
                content={"error": "Invalid JSON", "raw_output": parsed},
                status_code=500,
            )
    except Exception as e:
        print(f"run_smol_specific: {str(e)}")
